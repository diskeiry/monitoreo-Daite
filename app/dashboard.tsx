...
export default function Dashboard() {
  ...
  // New states for navigation to specific client/tab
  ...
  useEffect(() => {
    const handler = (event: CustomEvent<{ screen: string; clientId?: string; tab?: string }>) => {
      setActiveScreen(event.detail.screen)
      if (event.detail.clientId) {
        setNavigateToClientId(event.detail.clientId)
      } else {
        setNavigateToClientId(null)
      }
      if (event.detail.tab) {
        setNavigateToTab(event.detail.tab)
      } else {
  ...
  // Callback to clear navigation props after they've been consumed by the target screen
  const onNavigationConsumed = () => {
    setNavigateToClientId(null)
    setNavigateToTab(null)
  }
    ...
    <ProtectedRoute>
      <div className="flex h-screen overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          {/* Header */}
          <header
            className={`bg-white shadow-sm border-b px-6 py-2 fixed top-0 ${
              isMobile ? "left-0" : sidebarOpen ? "left-64" : "left-0"
            } right-0 z-10 transition-all `}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                {isMobile && <img src="/daite.svg" alt="Logo Daite" className="w-8 h-auto" />}
          ...
          <main className="flex-1 overflow-y-auto px-6 pb-6 pt-[45px]">
            {/* Pass navigation props to ClientsScreen */}
            {activeScreen === "clients" ? (
              <ClientsScreen
                initialClientId={navigateToClientId}
                initialTab={navigateToTab}
